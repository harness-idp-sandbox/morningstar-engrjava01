...
# run app
java -jar target/engrjava01-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/engrjava01:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/engrjava01 -n dev

# get external endpoint
kubectl get svc engrjava01-svc -n dev
```
...
```bash
kubectl port-forward svc/engrjava01-svc 8080:80 -n dev
...
kubectl rollout restart deploy/engrjava01 -n dev
kubectl rollout status deploy/engrjava01 -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/engrjava01/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© rohit.dalvi@morningstar.com. For demo/PoV purposes.
